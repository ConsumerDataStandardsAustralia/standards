...
```diff
Clarified formatting of RateString type and updated styling of Valid Examples
```
...
AmountString | A string representing an amount of currency.<br/>- A positive, zero or negative number<br/>- Negative numbers identified with a hyphen (`-`)<br/>- Currency symbols **MUST NOT** be supplied<br/>- At least 1 and up to a total of 16 significant digits before a decimal point<br/>- Minimum 2 digits following a decimal point (more digits allowable but only if required)<br/>- No additional formatting, e.g., thousand-separating commas. | `"0.01"`<br/>`"10.00"`<br/>`"1234567.89"`<br/>`"-1001.23"`<br/>`"1.999"`
