...
```diff
For NBL: Added product category with associated description
+ BUY_NOW_PAY_LATER
```
...
| BUY_NOW_PAY_LATER | This grouping of products includes 'Buy Now, Pay Later' products with key characteristics including: the provision of finance (excluding supplying cash), the involvement of a third-party financing entity, a fixed charge for providing credit under a prescribed limit instead of charging interest, and a fixed charge for missing a payment. |
