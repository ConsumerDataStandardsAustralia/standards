...
```diff
From 1.34.0: Added featureType:
+ FUNDS_AVAILABLE_AFTER

For NBL: Added featureType:
+ EXTRA_DOWN_PAYMENT
```
...
```diff
From 1.34.0: Added constraintType:
+ MAX_LVR
+ MIN_LVR
+ OTHER
```
...
```diff
From 1.34.0: Updated eligibilityType 'Use of additionalValue Field' details:
...
```diff
From 1.34.0: Updated feeType values:
- VARIABLE
- PAYMENT_LATE
+ CASH_ADVANCE
+ DISHONOUR
...
For NBL: 
 - UPFRONT_PER_PLAN feeType was present in the original candidate and is now also in the binding standards
```
...
```diff
From 1.34.0: Updated discountEligibilityType 'Use of additionalValue Field' details:
...
```diff
From 1.34.0: Updated descriptions of Deposit Adjustment Rate Types
```
...
```diff
From 1.34.0: Added lendingRateType:
+ BALANCE_TRANSFER
```
...
```diff
From 1.34.0: Updated descriptions of Lending Adjustment Rate Types
```
...
```diff
From 1.34.0: Added Rate and Tier Applicability Types section
```
