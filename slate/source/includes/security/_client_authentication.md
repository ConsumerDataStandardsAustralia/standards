...
1. self-signed JWT client assertion authenticated by the protected request endpoint, or
2. private_key_jwt authentication by the Data Holder Authorisation Server as specified at section 9 of **[OIDC]**;

Data Recipients MUST support the authentication of the CDR Register using the self-signed JWT Client Authentication method.
...
* The aforementioned assertion MUST be sent to the Data Holder's Token endpoint with the `POST` method and MUST include the following parameters:

    * `grant_type`: This parameter MUST only be included when invoking the Token End point and MUST be set to `client_credentials`.  
    * `client_id`: The client ID of the bearer.
    ...
    * `scope`: The scope of the access request as described by [section 3.3](https://tools.ietf.org/html/rfc6749#section-3.3) of **[RFC6749]**.

* Upon successful authentication, Data Holders MUST issue an Access Token to the CDR Register but MUST NOT issue a Refresh Token in accorance with [section 4.4](https://tools.ietf.org/html/rfc6749#section-4.4) of **[RFC6749]**.
...
*	The JWT MUST be accepted from the client at the requested endpoint using the "Authorization Request Header Field" mechanism as described in [section 2.1 of RFC6750](https://tools.ietf.org/html/rfc6750)
